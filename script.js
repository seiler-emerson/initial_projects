...
function press(n) {  //Para cada pixel selecionado
    let verificacao = document.querySelector(`.pixel_${n}`).classList.contains(`write`);  //A variável verificacao recebe o pixel selecionado e verifica se contain a class write responsável pela cor do pixel

    if (verificacao === false) {                                                          //Caso a variavel não tenha a class write, ela não está pintada
...
button.addEventListener('click', limpar)               //No botão é adicionado um evento de click com uma função de nome "limpar"
